...
lazy_static! {
    pub static ref DB: AsyncOnce<Surreal<Db>> =
        AsyncOnce::new(async { connect_db().await.expect("Unable to connect to database") });
}

async fn connect_db() -> Result<Surreal<Db>, Box<dyn std::error::Error>> {
    // get directory of current binary
    let path: std::path::PathBuf = dirs::config_local_dir()
    ...
    db.query(
        "
            ...
            DEFINE FIELD id ON TABLE content TYPE string;
    ...
    db.query(
        "
            ...
            DEFINE FIELD id ON TABLE vector_index TYPE string;
            DEFINE FIELD content_id ON TABLE vector_index TYPE string;
...
impl VectorIndex {
    pub async fn get_adjacent_chunks(&self, mut upper: u16, lower: u16) -> Result<Vec<VectorIndex>, Error> {
        let db = DB.get().await.clone();
...
pub async fn insert_content(title: &str, text: &str) -> Result<Content, Error> {
    ...
    let thing_id = thing(format!("content:{}", id).as_str())?;
...
pub async fn insert_vector_index(
    ...
    let db = DB.get().await.clone();
    let id = Uuid::new_v4().0.to_string().replace("-", "");
    let thing_id = thing(format!("vector_index:{}", id).as_str())?;

    let content_chunk = content_chunk.chars().filter(|c| c.is_ascii()).collect::<String>();
...
mod test {
    ...
    async fn setup() {
        ...
        let _ = forget_all_content().await;
        let _ = get_embeddings("Hello world").unwrap();
    }
    ...
    async fn test_insert_content() {
        ...
        let text = "Test Text";
        let content = insert_content(title, text).await.unwrap();

        assert_eq!(content.title, title);
        assert_eq!(content.text, text);
        assert!(content.id.to_string().starts_with("content:"));
    }

    #[tokio::test]
    async fn test_insert_vector_index() {
        setup().await;

        let content = insert_content("Test Title", "Test Text").await.unwrap();
        let chunk = "Test chunk";
        let vector_index = insert_vector_index(
            content.id.clone(),
            0,
            chunk,
            json!({ "content_id": content.id.to_string() }),
        )
        .await
        .unwrap();

        assert_eq!(vector_index.content_id, content.id.to_string());
        assert_eq!(vector_index.chunk_number, 0);
        assert_eq!(vector_index.content_chunk, chunk);
        assert_eq!(vector_index.vector.len(), 384); // embedding size
    }

    #[tokio::test]
    async fn test_smart_insert_content() {
        setup().await;

        let title = "Test Title";
        let text = "Test Text\nTest Text 2";
        let content = smart_insert_content(title, text, json!({ "information": "This is a test"}))
            .await
            .unwrap();
        ...
        // check the vector indexes was created
        // let vector_indexes = content.get_vector_indexes().await.unwrap();
        // assert_eq!(vector_indexes.len(), 3); // One per line
    }
}
