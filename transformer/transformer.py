...
The functions are implemented for a single sample and `jax.vit` can vectorize
...
class Module:
    ...
    It handles parameters and transforms methods to pure functions for
    JAX to compile and differentiate.

    You can skip these modules to get into the models directly.

    The modules stores parameters and sub-modules separately.
    When we want to transform any method to a pure function, we pass the
    parameters of the module and the sub-module as an argument and assign
    the passed values to class.
    """

    # Store all parameters and sub-modules in dictionaries
    _submodules: Dict[str, "Module"]
    _params: Dict[str, jtype.ArrayLike]
    ...
    def __setattr__(self, key: str, value: Any):
        ...
        elif isinstance(value, jtype.ArrayLike):
    ...
    def _clear_params(self):
        ...
        These clears out all the parameters. This is used when a method is
        called as a pure function. We first clears out all the parameters
        and assigns the parameters passed to the pure function.
        """
    ...
    def get_params(self) -> Dict[str, jtype.ArrayLike]:
        """Collect all the parameters

        This recursively collects all the parameters of the module
        and sub-modules into a dictionary.
        """
    ...
    def _set_params(self, params: Dict[str, jtype.ArrayLike]):
        """
    ...
    def _set_param(self, param_path: List[str], value: jtype.ArrayLike):
        """
    ...
    def purify(self, method: Callable) -> Callable:
        ...
        This transforms a member method to a pure function that accepts
...
class ModuleList(Module, Generic[M]):
    ...
    This stores a list of modules. We needed this for transformer decoder to
    hold the list of transformer layers.
    """
...
class EmbeddingsWithLearnedPositionalEncoding(Module):
    ...
    def __call__(self, x: jax.Array):
        ...
        return jax.Array(self.embeddings(x) * self.pe_coef + pe)
...
class LayerNorm(Module):
    ...
    def __call__(self, x: jtype.ArrayLike) -> jax.Array:
        ...
        mean_2 = (x**2).mean(axis=tuple(axes), keepdims=True)
...
class PrepareForMultiHeadAttention(Module):
    ...
    def __call__(self, x: jtype.ArrayLike) -> jax.Array:
        ...
        x = jax.Array(x.reshape(*head_shape, self.heads, self.d_k))
...
class TransformerLayer(Module):
    ...
    This is a transformer layer with multi-head attention and a position-wise
    feed-forward layer. We use pre-layer layer normalization.
    """
    ...
    def __init__(
        ...
        """
        * `size_model` is the token embedding size
        ...
        * `feed_forward` is the feed forward module
        ...
        self.norm_self_attn = LayerNorm([size_model])
        self.norm_ff = LayerNorm([size_model])

    def __call__(self, x: jtype.ArrayLike, mask: jtype.ArrayLike) -> jax.Array:
...
class AutoregressiveTransformer(Module):
    ...
    def __init__(
        ...
        * `d_ff` is the number of features in the hidden layer of the FFN
        ...
        self.embeddings = EmbeddingsWithLearnedPositionalEncoding(
            emb_key, n_vocab, d_model
        )
...
class Adam:
    ...
    def _init_state(self, param: jtype.ArrayLike) -> AdamState:
        """
    ...
    def _step(self, n_steps: int, param: jtype.ArrayLike, state: AdamState):
        """
        ### Update parameters
...
def main():
    ...
    model = AutoregressiveTransformer(
        rnd_key, dataset.n_tokens, d_model=128, n_layers=3, heads=8, d_ff=512
    )
    ...
    pure_forward_fn = jax.jit(
        jax.vmap(model.purify(model.__call__), in_axes=(None, 0), out_axes=0)
    )
    # Similarly we vectorize loss computation
    pure_loss_fn = jax.jit(
        jax.vmap(model.purify(model.get_loss), in_axes=(None, 0), out_axes=0)
    )
